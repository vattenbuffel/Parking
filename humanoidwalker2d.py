...
import sys
sys.path.append("/home/noa/Documents/Parking")
from ParkingSimulator import ParkingSimulator
envs = ParkingSimulator()
env = ParkingSimulator()
# envs = gym.make('Pendulum-v0')
...
# state = envs.reset().reshape(1, 3) # Pendulum
state = envs.reset().reshape(1,1)
early_stop = False
t_s = time.time()

while not early_stop:
    print(f"Iteration took: {time.time() - t_s} s")
    t_s = time.time()
    ...
    for balle in range(num_steps):
        ...
        masks.append(torch.tensor(np.array([1-done]), dtype=torch.float).to(device))
        
        states.append(state)
        actions.append(action)
        
        # next_state = next_state.reshape(1, 3) # Pendulum
        ...
        if frame_idx % 1000 == 0:
            ...
            distance_travelleds = []
            for i in range(10):
                reward, distance_travelled = test_env(vis=i==0)
            ...
            print(f"score: {test_reward:.2f}, distance travelled: {distance_travelled:.2f}")
